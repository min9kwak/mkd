...
from configs.simulation import ConfigSimulationDisc
from tasks.simulation.simulator_disc import SimulatorDisc
...
def main():
    ...
    config = ConfigSimulationDisc.parse_arguments()
...
def main_worker(local_rank: int, config: argparse.Namespace):
    ...
    if config.enable_wandb:
        wandb.init(
            ...
            project='incomplete-kd-simulation-disc',
    ...
    loss_function_disc = nn.CrossEntropyLoss(reduction='mean')
    ...
    model.prepare(config=config,
                  ...
                  loss_function_disc=loss_function_disc,
