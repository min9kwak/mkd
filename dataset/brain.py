...
class BrainProcessor(object):
    ...
    def process(self, validation_size: float = 0.1, test_size: float = 0.1, missing_rate: float = None):
        ...
        data['PET_available'] = ~data[self.pet_type].isna()
        ...
        if missing_rate is not None:
            ...
            data_train.loc[adjust_index, self.pet_type] = np.nan
    ...
    def parse_data(self, data):
        ...
        pet_files = [self.str2pet(p) if type(p) == str else p for p in data[self.pet_type]]
...
class Brain(BrainBase):
    ...
    def __init__(self, dataset, data_type, mri_transform, pet_transform):
        super().__init__(dataset)
        assert data_type in ['mri', 'pet', 'multi']
        self.data_type = data_type
        self.mri_transform = mri_transform
        self.pet_transform = pet_transform

    def __getitem__(self, idx):
        mri, pet = np.nan, np.nan
        if self.data_type in ['mri', 'multi']:
            mri = self.load_image(path=self.mri[idx])
            if self.mri_transform is not None:
                mri = self.mri_transform(mri)
        if self.data_type in ['pet', 'multi']:
            pet = self.load_image(path=self.pet[idx])
            if self.pet_transform is not None:
                pet = self.pet_transform(pet)
        demo = self.demo[idx]
        y = self.y[idx]
        return dict(mri=mri, pet=pet, demo=demo, y=y, idx=idx)


if __name__ == '__main__':
    from torch.utils.data import DataLoader
    from dataset.transforms import make_mri_transforms
    ...
    train_transform, test_transform = make_mri_transforms(image_size=96)

    mri_set = Brain(mri_pet_complete_train, 'mri', None, None)

    mri_loader = DataLoader(mri_set, batch_size=1)
    M = -100
    for i, batch in enumerate(mri_loader):
        if i == 3:
            a = torch.flatten(batch['mri']).numpy()
            a = a[a>0]
