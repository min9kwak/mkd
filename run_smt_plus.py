...
from configs.slice.smt_plus import SMTPlusConfig
from tasks.slice.smt_plus import SMTPlus
...
def main():
    """Main function for training SMT⁺ (enhanced teacher via mutual transfer)."""

    # Parse configuration arguments
    config = SMTPlusConfig.parse_arguments()
...
def main_worker(local_rank: int, config: argparse.Namespace):
    ...
    # Load teacher weights from original SMT checkpoint (optional)
    ...
    # Model (Task)
    model = SMTPlus(networks=networks)
