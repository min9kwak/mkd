...
class GeneralDistillation(object):
    ...
    network_names = [
        # Teacher
        'extractor_mri', 'extractor_pet', 'projector_mri', 'projector_pet',
        'encoder_general', 'encoder_mri', 'encoder_pet',
        'decoder_mri', 'decoder_pet', 'transformer_encoder', 'classifier',

        # Student
        'extractor_mri_s', 'projector_mri_s', 'encoder_mri_s', 'transformer_encoder_s', 'classifier_s'
    ]
    ...
    def prepare(self,
        ...
        names = [name for name in self.networks.keys() if not name.endswith('_s')]
        for name in names:
    ...
    def _set_learning_phase(self, network_type:str, train: bool = True):
        ...
        if network_type == 'student':
            names = [name for name in self.networks.keys() if name.endswith('_s')]
        else:
            names = [name for name in self.networks.keys() if not name.endswith('_s')]
