...
class SMT(object):
    """Student-oriented Multi-modal Teacher (SMT).
    
    Trains a multi-modal model (MRI+PET) to extract common representations
    via modality disentanglement. Uses similarity loss to encourage common features,
    difference loss to separate modality-specific features, and reconstruction loss
    ...
    def prepare(self,
                ...
                local_rank: int = 0,
                **kwargs):
        """Prepare model for training by setting up losses, optimizers, and configurations."""

        # Store configuration and training settings
        self.config = config

        # Store loss functions for different training objectives
        self.loss_function_ce = loss_function_ce  # Classification
        self.loss_function_sim = loss_function_sim  # Common representation similarity
        self.loss_function_diff = loss_function_diff  # Modality-specific difference
        ...
        # Determine number of test slices based on training mode
    ...
    def run(self, datasets, save_every: int = 20, **kwargs):
        ...
        # Create data samplers to handle class imbalance
        train_sampler = None
        ...
        # Initialize data loaders for train/validation/test
